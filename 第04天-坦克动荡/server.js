 ...
 * - 先拿 5 分者赢整场；单把上限 300s，超时双方不得分进下一把
 * - 比分 4:4 后下一把为决胜把
...
class Room {
  ...
  tick() {
    ...
    for (let i = this.bullets.length - 1; i >= 0; i--) {
      ...
      let dead = b.life <= 0;
      let hitP = null;
      // 直射无效反馈：未反弹的炮弹穿过坦克时，推一个"无效穿透"事件（客户端提示"直射无效"），每发弹只报一次
      const noEffectTest = () => {
        if (b.bounces >= 1 || b._noHit) return;
        for (const p of this.players.values()) {
          if (!p.alive || p.waitingNextRound) continue;
          if (p.invuln > 0) continue;
          if (p.id === b.ownerId && b.age < MUZZLE_PROTECT) continue;
          if (Math.abs(b.x - p.x) < HIT_R && Math.abs(b.z - p.z) < HIT_R) {
            b._noHit = true;
            this.events.push({ kind: 'no_effect', x: r2(p.x), z: r2(p.z), targetId: p.id, ownerId: b.ownerId });
            return;
          }
        }
      };
